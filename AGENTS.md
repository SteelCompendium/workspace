...
- `steel-etl/` -- Go CLI tool: the primary ETL pipeline and site builder (own AGENTS.md)
...
- **This file (and each sub-repo's AGENTS.md) is a router: current state + pointers only,
...
AGENTS.md is the canonical instruction file for Codex and Claude Code. Keep each
CLAUDE.md as a one-line `@AGENTS.md` import; edit shared guidance in AGENTS.md.

Before working in a sub-repo, read its AGENTS.md if present, in addition to this
workspace guidance. Keep repo-specific context in that sub-repo's AGENTS.md.
Claude-specific plugin and skill paths under `.claude/` remain tool-specific;
Codex should use its available tools and skills to follow the shared guidance.
