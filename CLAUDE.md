...
- **Companions** (beastheart) live in the `monster.companion.beastheart.*` family (container `monster.companion.beastheart.statblock/<species>`; members `feature[.ability].companion.beastheart.<species>.level-N/<id>`), mirroring the Monsters-book Rivals (restructured from the old `feature-group.companion/*` on 2026-06-13, Plan 5a — see `docs/scc-log.md`). Embeddable advancement-features entities + fixture parity are Plan 5b/5c.
