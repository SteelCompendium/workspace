...
**Registry:** 2,627 codes across three books — heroes (1,808), beastheart (228), and monsters (591). The `project` and `god` types were added 2026-05-29 during the truncated-link fix (see `steel-etl/docs/superpowers/plans/2026-05-29-truncated-link-fix.md`); treasures were reorganized into a `treasure/<tier>/<category>/<item>` hierarchy 2026-06-04 via the non-code-producing `treasure-group` container (see `steel-etl/docs/superpowers/plans/2026-06-04-treasure-hierarchy.md`); the **Monsters book** was integrated 2026-06-05 with a nested `monster/<category>/statblock/<item>` hierarchy (malice featureblocks as siblings of the `statblock/` folder), plus `dynamic-terrain` and `retainer` types — see `steel-etl/docs/superpowers/plans/2026-06-05-monsters-book.md`. The heroes source document has ~4,055 SCC cross-reference links including conditions, skills, movement types, negotiation motivations, and culture benefits. See `steel-etl/docs/linking-guide.md` for linking rules and `steel-etl/docs/linking-reference.md` for all 441 linkable terms.
