...
When resolving `scc:` links into the `*-linked` outputs, the resolver honours the registry's `scheme_version`: it accepts an optional `scc.vN:` prefix (bare `scc:` ≡ `scc.v1`) and a reserved `#format` qualifier, strips the qualifier to the canonical identity, and leaves a mismatched-version link unresolved rather than binding it to v1 content. Format is never part of SCC identity. Scheme rules: [`docs/scc-reference.md`](docs/scc-reference.md) and `reference/scc-specification.md` §2.0/§8/§9; resolver mechanics: `steel-etl/CLAUDE.md` → "Scheme version".
...
Key operations:
- **Section mapping** -- copies content into `Browse/` and `Read/` tab directories. The `Read/` tab is grouped per book (`Read/<book>/`, configured via `books:` in `v2/site.yaml`) and chapters are ordered by their position in the source document (`order:` frontmatter), not alphabetically. The modular Monsters-book pages (`monster/<category>/…`, `dynamic-terrain/…`, `retainer/…`) live on the **`Browse/`** tab; the book-faithful everything-inline monster view stays on `Read/`'s `chapter/monsters` page. The redundant `statblock/` folder is hoisted out of the site URL (`hoistStatblockPath`) while the SCC codes keep their `.statblock` segment — **code≠path**. The **`Bestiary/` tab is a client-side Search & Filter utility**: `bestiary_search.go` emits a `.sc-bestiary-mount` JSON data island that `v2/docs/javascripts/steel-bestiary-browser.js` mounts into search + facets + a sortable table. All of this is SITE-ONLY — read from existing frontmatter, no SCC re-mint, no data-repo change. Group-landing assembly, card renderers, and the per-file map: `steel-etl/docs/site-builder.md`; statblock placement + the code≠path hoist: `steel-etl/docs/statblocks.md`. Monsters generate to `data/data-unified/en/books/monsters/` (see the multi-book gotcha above; `just deploy*` passes `--all`).
- **Search coverage** -- Browse is fully indexed; Read is indexed only where no Browse page carries the text. `search_coverage.go` marks every Read heading whose section (or an ancestor's) has a Browse page with `data-search-exclude=""` (`v2/site.yaml` `search_uncovered_only: [Read]`, SC-329); Material opens an index section per heading, so each covered heading is marked. A section that later gains a Browse page drops out of Read's index automatically. Not quite every word: a covered container page's own intro prose (e.g. a Monsters group landing's per-echelon introduction) is excluded from Read without a Browse-page equivalent to replace it — findable nowhere; deferred to Backlog SC-345.
...
- **SCC permalink stubs** -- generates `scc/{code}/index.html` redirect files. The SCC URL is a stable, shareable redirect entry point; the friendly Browse page is the canonical, indexable location. (Do not reintroduce a client-side address-bar rewrite — `v2/.repo-docs/decisions/2026-05-31-retire-scc-address-bar-rewrite.md`.)
...
**Downtime projects:** `project.go` promotes the four project requirements into optional string fields in all structured outputs; qualifiers and links remain intact. Both schema copies include `project.schema.json`. Untyped quoted H6+ headings stay in their owner’s body; only typed quoted entities open parser sections. Site project cards preserve grant ownership through nested ability cards (see `steel-etl/docs/site-builder.md` → “Downtime project cards”).
