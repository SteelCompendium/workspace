...
| Class landing header (`.sc-classhead` 6-slot card + potency strip + `.sc-classnav` H2 jump bar on every Browse class page) | build-time `internal/site/class_page.go` + `v2/docs/stylesheets/steel-class.css` | `docs/superpowers/plans/2026-07-01-p3-class-landing-header.md` |
| "My Table" pinboard (hover-revealed ★ in the card top-center control strip + inline ★ beside prose H1s; `/pins/` board, localStorage, 200-item cap) | `v2/docs/javascripts/sc-pins-core.js`/`sc-pins.js` + `steel-pins.css`; board page `static_content/docs/pins.md` | `docs/superpowers/plans/2026-07-01-p7-pinboard.md` |
| Encounter builder (EV budget tray on `/Bestiary/` + add-from-statblock chip; book bands Trivial→Extreme, minions-by-four, `?enc=` share links) | `sc-encounter-core.js`/`sc-encounter.js` + `steel-encounter.css`; "+" column + `window.SC_BESTIARY_ITEMS` seam in `steel-bestiary-browser.js` | `docs/superpowers/plans/2026-07-01-p8-encounter-builder.md` |
...
| Statblock level scaler ("Scale to level" stepper; book "Adjusting Monster Levels" formulas applied as deltas from printed values; dashed-outline + banner while scaled; never persists) | `sc-scale-core.js`/`sc-scale.js` + `steel-scale.css` | `docs/superpowers/plans/2026-07-01-p11-statblock-scaler.md` |
