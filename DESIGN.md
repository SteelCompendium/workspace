...
| Featureblocks (`.fb-wrap` Forged Band card — titled collection of Features under a loose-stat header; statblock-like in anatomy, not rigor; also renders Summoner fixture statblocks via `internal/site/fixture_page.go`, and retainer advancement abilities as an "Advancement Abilities" card below the statblock island via `internal/site/retainer_page.go`) | build-time `internal/site/featureblock_page.go` + `v2/docs/stylesheets/steel-featureblock.css` | `docs/superpowers/specs/2026-06-12-featureblock-cards-design.md` |
