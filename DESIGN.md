...
per-card page actions (copy-link at `left:50%`, pin at `+1.6rem`, encounter-add at
`+3.4rem`, MD/PNG exports at `+5.2rem`), hover-revealed on the card — never place
controls in the head grid's column 3 (they collide with the Level chip / role mini). The statblock head already embodies the model (`Level / Org+Role /
...
| Class landing header (`.sc-classhead` 6-slot card + in-card flavor paragraph, base-stat cells (starting characteristics/stamina/per-level/recoveries), potency strip, skills footer; `.sc-classnav` H2 jump bar with the ten level headings collapsed into one "Level 1…10" group) | build-time `internal/site/class_page.go` (+ `internal/content/class.go` frontmatter emit) + `v2/docs/stylesheets/steel-class.css` | `docs/superpowers/plans/2026-07-01-p3-class-landing-header.md` |
| "My Table" pinboard (Material pushpin — outline, filled when pinned; hover-revealed in the card top-center control strip AND beside prose H1s like the ¶ permalink; `/pins/` board, localStorage, 200-item cap) | `v2/docs/javascripts/sc-pins-core.js`/`sc-pins.js` + `steel-pins.css`; board page `static_content/docs/pins.md` | `docs/superpowers/plans/2026-07-01-p7-pinboard.md` |
| Encounter builder (EV budget tray on `/Bestiary/` + add-from-statblock chip; book bands Trivial→Extreme, minions-by-four, `?enc=` share links; header ⋯ menu: clear / reset / session-close) | `sc-encounter-core.js`/`sc-encounter.js` + `steel-encounter.css`; "+" column + `window.SC_BESTIARY_ITEMS` seam in `steel-bestiary-browser.js` | `docs/superpowers/plans/2026-07-01-p8-encounter-builder.md` |
...
| Statblock level scaler ("Scale to level" stepper; book "Adjusting Monster Levels" formulas applied as deltas from printed values — EV/Stamina/damage/free strike, characteristics (±5 clamp), "Power Roll +N" bonuses (dice popover follows), and entity-aware potency rewrites in tier rows and effect prose; dashed-outline + banner while scaled; never persists) | `sc-scale-core.js`/`sc-scale.js` + `steel-scale.css` | `docs/superpowers/plans/2026-07-01-p11-statblock-scaler.md` |
