# SC-191 slice 3 brief — the reference surfaces: cheat-sheet strip + foot guide, badge fix, gold pip
...
4. Worktree — verify `pwd` before ANY write; never write under
   ...
   Branch `sc191-montage-overhaul`. Current tip: `7d4451a` (fix-1 commit; slice 2 was b2f696e) (slice 2 commit). Tracked branch: `origin/develop`.
...
**Slice 3 — the reference surfaces: cheat-sheet strip + foot guide.** (spec §I, slice 3)

Prerequisite: slices 1–2 are committed (slice 2 commit `7d4451a` (fix-1 commit; slice 2 was b2f696e)) and the review-1
fix round (if any) has landed; read `sc191-slice2-report.md` and `sc191-review1-report.md`
first. Build on the board; do not re-open §B or the slice-2 views beyond what §D says the
strip/guide need from them.

Scope (spec §I slice 3, verbatim):
...
Spec sections that define it: **§A** (design freeze), **§D** (strip + guide component
mapping: the shipped Power Roll `.dse-pr__badge` / `.dse-pr__row` reuse decision and the
Steel scoping rule), **§E** (CSS migration — tokens only, dark/light/print), **§F** (the two
interaction captures), **§J1** (badge fix — the exact rule to delete and the key-track
sizes `3.21em` / `2.85em` narrow; the acceptance measurement 51.25 × 22.14 px, padding
`4.608px 4.032px`), **§J2** (the pip — gold `--dse-vp` fill, `--dse-sheen-soft` sheen, 1 px
`--dse-metal-line` rim; ▲ reward / ▼ consequence; print `#8a6a00`). Read them fully.

Carry-over from fix round 1 (owner ruling, fold here — two one-test additions, no code
change expected): (a) a test pinning that `participants: []` is OMITTED on serialize
(review-1 L-3); (b) a note-sort test whose fixture actually distinguishes roster order from
alphabetical order (review-1 L-4). Read `sc191-fix1-report.md` "Follow-ups" for the exact
gap each describes.

Out of scope (do NOT touch): the ⋯ chrome menu, the sheet, per-cell edit/note, user docs,
CHANGELOG (slice 4). Do not regenerate `rebaseline.txt` here — slice 4 regenerates it from
the final tree; but DO report the freeze result exactly.
...
> "I think it would be nice to have some guidance available for how to run the montage,
...
> collapsed by default." (2026-08-28)

> "The collapsed rules guidance looks good.  I have the feeling the "Each test" section
> will want to be seen all the time by a lot of Directors while the other rule information
> is a "check it once at the beginning and then close it" situation." (2026-08-29)

> "The `handle` design is great, lets go with that" (2026-08-29 19:46)
...
> table?" (2026-08-29 19:46) — resolved: flipped; wash is `edge`, not the shipped 60% `pr`
> fade; crit row stays.
...
> to actual power rolls.  I dont need to approve this change - just make it so." (2026-08-30)

> "I think the triangles are the strongest, but I dont love how basic they are.  Maybe some
> color?  Maybe a gradient? maybe a border?  Idk, pick a solid option." (2026-08-30) —
> resolved by spec §J2 (gold-filled forged tab, steel-grey rim). Implement §J2 exactly.
...
The approved look for this slice is `sc191-r7-pip-gold-dark.png` / `-light.png` /
`-grey-dark.png` and `sc191-r6-pip-narrow-dark.png` in the ledger dir; compare your captures
against them and report any deliberate difference with the reason.
...
- jest: 3579 passed / 1 skipped / 192 suites (post-fix-1, on 69eb5f7), **plus** your new tests
- shots: 498 PNGs (plus any new capture ids this slice adds), 0 FAIL, byte-identical
  across two runs
...
One commit per slice on `sc191-montage-overhaul`, message `SC-191 slice 3 — strip, guide, badges, gold pip`.
...
Write `/home/scott/code/steelCompendium/workspace/.superpowers/sdd/sc191-montage-overhaul/sc191-slice3-report.md`,
