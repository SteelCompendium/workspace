...
- **Context:**
  ...
  - SCC codes today map to **pages**, not sub-headings — the stub generator emits `scc/<code>/index.html` redirects per page (`steel-etl/internal/site/permalinks.go`). Per-heading SCC permalinks depend on heading-level SCC codes + anchor-qualified targets, which is the open work tracked in the **"In-page anchor links on class/chapter/ancestry pages"** followup below — coordinate the two.
  ...
  - Gotcha: link-dense pages were the cause of the ~31s render (link previews, since disabled). Adding an anchor + handler per heading is far lighter than per-link previews, but still verify it doesn't re-introduce per-navigation DOM-walk cost (see the v2 perf followup).
- **Effort:** S (page-title icon reuse only) / M (true per-heading SCC anchors, gated on the in-page-anchor work).
