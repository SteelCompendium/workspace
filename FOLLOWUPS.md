...
**Status:** in progress (infrastructure + glossary done 2026-06-07; first in-prose batch done 2026-06-08)
...
- **Remaining:** (1) **`damage` bare word — CONSERVATIVE only** (Scott's 2026-06-08 call, see memory `comprehensive-linking-density.md`): link only genuine rules-concept prose ("when a creature takes damage…"), **never** numeric rolls ("5 fire damage") or ability effect lines. This is a per-location curation, NOT a blanket regex — `link_apply` can't distinguish prose from statblock effect lines, so it must be done by reading the rules-prose sections (Basics health overview, Combat damage/health rules prose) and skipping all `>` blockquote statblock lines + the verb "to damage". (2) The **long tail of other `rule.*` terms** not yet swept in-prose: potency, speed, stability, power-roll, natural-roll, double-edge/bane, tier outcomes, combat-round, turn, main/maneuver/move actions, triggered-action, opportunity-attack, line-of-effect, critical-hit, area shapes (line/cube/wall/burst/aura), test/group-test/montage-test/test-difficulty, the downtime/negotiation/treasure/world/general groups, etc. (see `rule-term-mapping.md` `new-rule` rows). Same per-instance disambiguation discipline.
