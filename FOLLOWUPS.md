...
**Status:** open — ⚠️ **POTENTIALLY STALE.** Evidence below was captured 2026-06-13 against the *current* featureblock renderer, but there is an **ongoing effort to move the featureblock stuff around** (cf. FOLLOWUPS #7, the featureblock/fixture/retainer plans in `steel-etl/docs/statblocks.md`). That refactor may change the data flow or fix this incidentally. **Re-verify after that effort lands** before acting — the file paths, line numbers, and even the root cause may have shifted.
