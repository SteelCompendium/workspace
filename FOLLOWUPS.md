...
- **Remaining — the ultra-common words only** (each needs HEAVY per-instance disambiguation, mundane-vs-mechanic; do one at a time with full context review like `cover`/`strike`): combat `target`/`ally`/`enemy`/`adjacent`/`side`/`condition`/`line`/`wall`/`melee`/`ranged`/`turn`/`combat-round` (round); general `ability`/`creature`/`ground`/`supernatural`; dice `bonuses-and-penalties` (bonus/penalty); treasure `enhancement`/`implement`; downtime `guide`; world `Capital`/`Saint` (proper-noun-adjacent — "Saint X" names). The mapping flags most of these "link very sparingly". See `rule-term-mapping.md`.
