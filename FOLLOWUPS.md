...
- **Remaining — only the 5 ubiquitous words + 4 low-yield positional ones (the in-prose tail; every code is already linked via the glossary):**
  - **`creature`/`ability`/`target`/`ally`/`enemy`** — appear in nearly every statblock effect line, so per Scott's `damage` precedent + the mapping's "link very sparingly" these want the **CONSERVATIVE** treatment: hand-curate only the rules-concept prose (their def-section-adjacent explanations + Basics/Combat overview), NOT every effect line. Per-location, not a blanket regex (like the `damage` pass).
  - **`side`/`line`/`wall`/`ground`** — deliberately left: mundane-dominated ("outside"/"other side of the room", "line of effect" already its own code, physical walls, "the ground"). Only the rare clear mechanic uses are worth it ("your side" initiative, a Wall/Line area ability, the Ground-and-Ceiling rule) — low yield, high mislink risk; skippable like the generic terms in #6.
