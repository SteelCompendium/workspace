...
- **Remaining:** the **long tail of other `rule.*` terms** not yet swept in-prose: potency, speed, stability, power-roll/natural-roll, double-edge/double-bane, tier outcomes, combat-round, turn, main/maneuver/move actions, triggered-action, opportunity-attack, line-of-effect, critical-hit, area shapes (line/cube/wall/burst/aura), test/group-test/montage-test/test-difficulty, and the downtime/negotiation/treasure/world/general groups (see `rule-term-mapping.md` `new-rule` rows). Same per-instance disambiguation discipline.
