...
## 5. Dangling ability cross-links to subclass-named dirs (`feature/ability/<subclass>/<ability>`)

**Status:** open

- **Identified:** 2026-06-05, while verifying the CI build-time perf changes (`navigation.prune` / `fetch-depth`); these `mkdocs build` warnings are **pre-existing content gaps, unrelated to those changes** (the `fancy-footwork` instance also appeared in the pre-change baseline build).
- **What:** Several pages link to abilities via a **subclass-keyed** path that is never published: abilities live under their **class** dir (`Browse/feature/ability/<class>/…`, e.g. `tactician`, `fury`), but these links point at subclass-named dirs that don't exist. Observed dangling targets: `feature/ability/sniper/patient-shot.md`, `feature/ability/shining-armor/protective-attack.md`, `feature/ability/martial-artist/battle-grace.md`, and `feature/ability/swashbuckler/fancy-footwork.md`. None of `sniper`, `shining-armor`, `martial-artist`, `swashbuckler` exist under `v2/docs/Browse/feature/ability/` (only class dirs do), and none of the four target files exist anywhere in `v2/docs`. Distinct from followup #2 (which is the `feature-group/companion/*` gap).
- **Why it matters:** Clickable-but-404 cross-references on core class content; also persistent `mkdocs build` warnings that bury real ones (each appears on both the `Browse/class/<class>.md` page and the `Read/heroes/classes.md` aggregate; `fancy-footwork` is linked from `Read/heroes/treasures.md`).
- **Context:** Root is in **link generation upstream** (steel-etl), not the v2 builder — the cross-reference emits a `<subclass>` path segment where abilities are filed under `<class>`. Confirm the intended SCC/path scheme: are these abilities meant to be published under subclass dirs, or should the links resolve to the class dir? Reproduce via `mkdocs build` in `v2/` and grep the warnings; check `steel-etl` linking (`steel-etl/docs/linking-guide.md`, `linking-reference.md`) and how ability paths are keyed.
- **Effort:** S (trace the upstream link-keying once the intended scheme is confirmed)
